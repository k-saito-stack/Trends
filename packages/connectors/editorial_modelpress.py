"""Modelpress editorial connector using ranking and feed pages."""
...
URLS = (
    ("https://mdpr.jp/news/rss", "rss"),
    ("https://mdpr.jp/ranking", "ranking"),
    ("https://mdpr.jp/", "homepage"),
)
REQUEST_HEADERS = {
    "User-Agent": (
...
TITLE_RE = re.compile(
    ...
    r"|<h\d[^>]*class=[\"'][^\"']*(?:title|headline|ttl)[^\"']*[\"'][^>]*>([^<]+)</h\d>",
    re.IGNORECASE | re.DOTALL,
)


class EditorialModelpressConnector(BaseConnector):
    def __init__(self, **kwargs: Any) -> None:
        super().__init__(source_id="EDITORIAL_MODELPRESS", stability="B", **kwargs)
    ...
    def fetch(self) -> FetchResult:
        ...
        return FetchResult(error=" | ".join(errors[-3:]) if errors else "no modelpress data")
